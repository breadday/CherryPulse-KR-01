...
- `node --test web/board/board.test.js` 3 passed, 두 JS 파일의 `node --check`, `git diff --check` 통과. 별도 Windows 회귀로 Python 3.10.8 32비트 전체 pytest 159 passed, Ruff 검사·포맷, 가상 데모, `pip check` 통과. 격리 `.venv`가 없어 사용자 설치 Python으로 검사했다. 아래 9/25 추가 검증에서 headless Chrome 모바일/데스크톱 viewport와 상호작용도 확인했다.
