...
| D02·D04~D10 | 비활성 보드와 종목 등록·편집·보관, 손절 버전 초안, JSON/CSV 가져오기 및 JSON 내보내기 구현. 각 계획 완료 기준에는 미도달 | Q12 매수 패턴 명세, 실제 적용 흐름 및 단계별 계획 완료 기준 필요 |
...
- Node 검사 **8 passed**. Chrome 모바일 emulation에서 CSV 가상 파일 2행을
  가져와 저장·카드 렌더링까지 확인했다. Chrome headless에서 관심종목 JSON
