...
| D02·D04~D10 | 비활성 보드와 종목 등록·편집·검색·보관, 손절 버전, JSON/CSV 교환 및 전체 보드 백업·복원 구현. 각 계획 완료 기준에는 미도달 | Q12 매수 패턴 명세, 실제 적용 흐름 및 단계별 계획 완료 기준 필요 |
...
- 작업 시작 기준은 `main`의 `045a63d`, 작업 시작 시 트리는 깨끗했다.
- 현재 목록 범위 안에서 종목코드·종목명·링크·메모를 검색하며 관심/보관 필터와
  조합해 사용할 수 있다. 검색은 초안 데이터를 변경하지 않는다.
- Node 테스트 **9 passed**. Chrome 모바일·데스크톱 emulation에서 가로 넘침 없는
  레이아웃, 검색 결과 및 보관 탭 필터 조합을 확인했다.

## D02 전체 보드 백업·복원 — 2026-09-25

- 시작 기준: `main`/`origin/main`의 `9ef1470`; 작업 시작 시 브랜치 `main`, 작업 트리
  clean, rebase 진행 상태 없음.
- 전체 백업은 `schemaVersion: 1`로 종목·보관 상태·패턴 전 버전·활성 상태·연결을
