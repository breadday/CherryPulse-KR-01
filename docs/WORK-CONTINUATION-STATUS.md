...
| D02·D04~D10 | 비활성 보드와 종목 등록·편집·보관, 손절 버전 초안 및 JSON/CSV 가져오기 구현. 각 계획 완료 기준에는 미도달 | Q12 매수 패턴 명세, 실제 적용 흐름 및 단계별 계획 완료 기준 필요 |
...
- `node --test web/board/board.test.js` 7 passed, 세 JS 파일의 `node --check`, `git diff --check` 통과. 별도 Windows 회귀로 Python 3.10.8 32비트 전체 pytest 159 passed, Ruff 검사·포맷, 가상 데모, `pip check` 통과. 격리 `.venv`가 없어 사용자 설치 Python으로 검사했다. 아래 9/25 추가 검증에서 headless Chrome 모바일/데스크톱 viewport와 상호작용도 확인했다.
...
- Node 검사 **7 passed**. Chrome 모바일 emulation에서 CSV 가상 파일 2행을
  가져와 저장·카드 렌더링까지 확인했다. 최종 양 viewport에서 가로 넘침 없이
  레이아웃을 재확인했다. 구현·형식은 [D02 기록](D02-BOARD-PROGRESS.md).
