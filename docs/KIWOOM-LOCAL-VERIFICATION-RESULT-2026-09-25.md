...
로컬 Linux Python 3.12 분리 환경 관련 pytest **15 passed**, Ruff 검사·
포맷 검사, `git diff --check` 통과. Windows 3.10 32비트 전체 검사와
키움 실제 다중 페이지 수신은 아직 실행하지 않았다. 해당 PC에서 최신
커밋을 받은 뒤 전체 검사를 재실행해야 한다.
