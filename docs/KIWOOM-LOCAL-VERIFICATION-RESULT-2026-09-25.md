...
읽기 전용 캡처 완료 판정 전에 페이지 번호·연속조회 표식·완료 플래그·원문
타입과 응답 행 구조를 검사한다. 행은 문자열 필드명과 문자열 값만 허용한다.
잘못된 구조는 `QUARANTINED`로 분류하고 정규화 행을 비운다. 이 검사는
구조적 적합성만 확인하며 필드 업무 의미를 검증하거나 보유·체결 사실로
승격하지 않는다.

Python 3.10.8 32비트에서 관련 검사 **28 passed**, 전체 pytest **159
passed**, `ruff check .`, `ruff format --check .`, 가상 데모 및
`git diff --check`가 통과했다. 키움 조회·주문, 운영 DB 변경, 배포는 하지
않았다.

## 읽기 전용 캡처 시각 일관성 검사

완료된 캡처라도 요청 시작·종료 시각과 페이지 수신 시각이 순서에 맞지
않거나 timezone 정보가 없는 경우 `QUARANTINED`로 분류하도록 보강했다.
모든 페이지 수신 시각은 요청 구간 안에 있어야 하고 페이지 순서대로
비감소해야 한다. 이런 캡처는 화면용 행으로 정규화하지 않는다. 이는
수집된 메타데이터의 일관성 검사이며 키움 서버 시각의 신뢰성이나 실제
응답 내용의 유효성을 입증하지 않는다.

검사: Python 3.10 32비트에서 읽기 전용 어댑터 검사 **25 passed**, 전체
pytest **155 passed**, `ruff check .`, `ruff format --check .`, 가상 데모,
`git diff --check` 통과. 이번 변경은 오프라인 소프트웨어 검사이며 키움
조회·주문, 운영 DB, 배포를 수행하지 않았다.
